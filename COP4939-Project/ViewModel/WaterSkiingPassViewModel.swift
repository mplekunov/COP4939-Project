...
class WaterSkiingPassViewModel : ObservableObject {
    ...
    init(waterSkiingCourseViewModel: WaterSkiingCourseViewModel<WaterSkiingCourseFromVideo>, cameraViewModel: CameraViewModel, sessionViewModel: BaseSessionViewModel) {
        ...
        Publishers.CombineLatest3(waterSkiingCourseViewModel.$course, cameraViewModel.$videoFile, sessionViewModel.$session)
            ...
            .compactMap { course, videoFile, session in
                ...
                return processor.process(course: course, records: session.data, video: videoFile)
            }
            .assign(to: &$pass)
    }
